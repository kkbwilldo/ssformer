...
class conv(nn.Module):
    ...
    def __init__(self, input_dim=512, embed_dim=768):
        ...
        self.proj = nn.Sequential(nn.Conv2d(input_dim, embed_dim, 3, padding=1, bias=False),
                                  build_norm_layer(dict(type='BN', requires_grad=True), embed_dim)[1], nn.ReLU(),
...
class simVit_pup(nn.Module):
    def __init__(self, class_num=2):
        super(simVit_pup, self).__init__()
...
simVit_pup = simVit_pup(class_num=1)
summary(simVit_pup, (1, 3, 512, 512))
from thop import profile
import torch

input = torch.randn(1, 3, 352, 352).to('cuda')
macs, params = profile(simVit_pup, inputs=(input,))
